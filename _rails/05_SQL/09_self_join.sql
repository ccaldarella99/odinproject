...
WHERE stops.name LIKE 'Craiglockhart'
...
JOIN stops stopa ON (a.stop=stopa.id)
JOIN stops stopb ON (b.stop=stopb.id)
WHERE stopa.name LIKE 'Haymarket'
AND stopb.name LIKE 'Leith'
...
JOIN stops stopa ON (a.stop=stopa.id)
JOIN stops stopb ON (b.stop=stopb.id)
WHERE stopa.name LIKE 'Craiglockhart'
AND stopb.name LIKE 'Tollcross'
...
JOIN stops stopa ON (a.stop=stopa.id)
JOIN stops stopb ON (b.stop=stopb.id)
WHERE stopb.name LIKE 'Craiglockhart'
AND a.company LIKE 'LRT'
...
JOIN stops stop ON stop.id=d.stop
where start.name LIKE 'Craiglockhart'
AND stop.name LIKE 'Sighthill'
